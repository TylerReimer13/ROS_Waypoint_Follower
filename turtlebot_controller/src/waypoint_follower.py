...
class WaypointFollower:
    def __init__(self, full_waypoints=None, init_x=0., init_y=0., init_theta=0., kp=5., ka=5.):
        self.x = init_x
        self.y = init_y
    ...
    def __call__(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.idx >= len(self.xdata):
                ...
                if np.linalg.norm(np.array([self.x, self.y])-np.array([self.ref_x, self.ref_y])) <= self.waypoint_thresh:
                    vel_msg.linear.x = 0.
                    vel_msg.angular.z = 0.
                    pub.publish(vel_msg)
